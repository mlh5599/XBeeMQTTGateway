...
def ResetXBee(configuration_manager, pi):
    if configuration_manager.reset_pin >= 0:
            #Ensure UART is in the correct mode
        pi.set_mode(14, pigpio.ALT0)
        pi.set_mode(15, pigpio.ALT0)

            #Reset the xbee for good measure
        pi.set_mode(configuration_manager.reset_pin, pigpio.OUTPUT)
        pi.write(configuration_manager.reset_pin,pigpio.LOW)
        time.sleep(1)
        pi.write(configuration_manager.reset_pin,pigpio.HIGH)
        time.sleep(2)
