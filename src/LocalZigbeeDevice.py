...
def Initialize(config_manager):
    ...
    try:
        ...
        logging.debug(f"Begin session on port {config_manager.device_port}")
        logging.debug("Baud rate %s" % config_manager.device_baud_rate)

        device = XBeeDevice(config_manager.device_port,
                            config_manager.device_baud_rate)
        device.open()
...
def SetPanID(device, configuration_manager):
    ...
    if (device.get_pan_id() != configuration_manager.coordinator_pan_id):
        logging.debug(
            f'Setting PAN ID to \
                {configuration_manager.coordinator_pan_id}, \
                    current ID is {device.get_pan_id()}')
...
def SetScanChannels(device, configuration_manager):
    """
    Sets the scan channels of the local Zigbee device to the
    coordinator scan channels.
    ...
    """
    device_updated = False
    if (device.get_scanning_channels() !=
       configuration_manager.coordinator_scan_channels):
        logging.debug(
            f"Setting scan channels to \
                 {configuration_manager.coordinator_scan_channels}")
...
def SetEncryptionOptions(device, configuration_manager):
    """
    Sets the encryption options on the local Zigbee device to the
    coordinator encryption options.
    ...
    """
    device_updated = False
    if (device.get_encryption_options() !=
       configuration_manager.coordinator_encryption_options):
        logging.debug(
            f"Setting encryption options to \
                 {configuration_manager.coordinator_encryption_options}")
        encryption_option_bytes = bytes.fromhex(
            configuration_manager.coordinator_encryption_options)
...
def GetEncryptionEnabled(device, configuration_manager):
    """
    Sets the encryption enabled flag on the local Zigbee device to the
    coordinator encryption enabled flag.
    ...
    device_updated = False
    if (device.get_encryption_enabled() !=
       configuration_manager.coordinator_encryption_enable):
        logging.debug("Enabling encryption")
        device.set_encryption_enabled(
            configuration_manager.coordinator_encryption_enable)
        device_updated = True
    return device_updated
...
def SetNodeID(device, configuration_manager):
    """
        Set the node ID to the value in the configuration manager if
        it is not already set

        Arguments:
            device {XBeeDevice} -- The device to set the node ID on
            configuration_manager {ConfigurationManager}:
            The configuration manager to get the node ID from

        Returns:
            bool -- True if the node ID was changed, false otherwise
    """
    device_updated = False
    if (device.get_node_id() !=
       configuration_manager.coordinator_node_identifier):
