...
class ConfigurationManager:
    ...
    def coordinator_baud_rate(self):
        """
         the serial interface baud rate for communication between modem serial
         port and host. Request non-standard baud rates above 0x12C using a
         terminal window.
    ...
    def coordinator_pan_id(self):
        """
        Set the PAN (Personal Area Network) ID for the network
    ...
    def coordinator_scan_channels(self):
        ...
        """
        return self.app_config.get("coordinator", {}).get(
            "scan_channels", "FFFF")

    @property
    def coordinator_scan_duration(self):
        """
        Set/read the Scan Duration exponent. The exponent configures the
        duration of the active scan and energy scan during coordinator
        initialization.
    ...
    def coordinator_zigbee_stack_profile(self):
        ...
        0=Network Specific, 1=ZigBee-2006, 2=ZigBee-PRO
        Range: 0x0 - 0x2 (Default: 0)
        """
        return self.app_config.get("coordinator", {}).get(
            "zigbee_stack_profile", "0")

    @property
    def coordinator_node_join_time(self):
        """
        Set/read the Node Join time. The value of NJ determines the time
        (in seconds) that the device will allow other devices to join to it.
    ...
    def coordinator_device_type_identifier(self):
        ...
        """
        return self.app_config.get("coordinator", {}).get(
            "device_type_identifier", "3000")

    @property
    def coordinator_node_discovery_backoff(self):
        """
        Set/read Node Discovery backoff register.
        This sets the maximum delay for Node Discovery responses to be sent
        (ND, DN).
        Range 0x20 - 0xFF (Default: 3C)
        """
        return self.app_config.get("coordinator", {}).get(
            "node_discovery_backoff", "3C")

    @property
    def coordinator_node_doscovery_options(self):
        """
        Sets the node discovery options register. Options include
        0x01 - Append DD value to end of node discovery.
        0x02 - Return devices own ND response first.
        Range 0x0 - 0x3 (Default: 0)
        """
        return self.app_config.get("coordinator", {}).get(
            "node_discovery_options", "0")

    @property
    def coordinator_pan_conflict_threshold(self):
        """
        Set/read threshold for the number of PAN id conflict reports that must
        be received by the network manager within one minute to trigger a PAN
        id change.
    ...
    def coordinator_power_mode(self):
        ...
        sensitivity by 1dB and increases output power by 2dB, improving the
    ...
    def coordinator_network_encryption_key(self):
        ...
        If set to 0 (default), the coordinator selects a random network
        encryption key (recommended). This register can not be read.
        0 - 32 hexadecimal characters (Default: '')
        """
        return self.app_config.get("coordinator", {}).get(
    ...
    def coordinator_DIO7_configuration(self):
        ...
        Options include: CTS flow control, Digital Input and Output,
        or RS-485 enable control.
        0 = Disable
        1 = CTS flow control
        2 = N/A
        3 = Digital Input
        ...
        """
        return self.app_config.get("coordinator", {}).get(
            "DIO7_configuration", "1")

    @property
    def coordinator_DIO6_configuration(self):
        """
        Configure options for the DIO6 line of the module.
        Options include: RTS flow control, Digital Input and Output,
        or RS-485 direction control.
        0 = Disable
        1 = CTS flow control
        2 = N/A
        3 = Digital Input
    ...
    def coordinator_API_output_mode(self):
        """
        Set the API output mode register value. 0 - Received Data formatted
        as native API frame format. 1 - Received RF data formatted as
        Explicit Rx-Indicator. 3 - Same as 1,
    ...
    def coordinator_cyclic_sleep_period(self):
        """
        Set/read Cyclic sleep period for cyclic sleeping remotes. Set SP on
        parent (Coordinator or Router) to match the largest SP of its end
        device children. On a router or coordinator, SP determines the
        transmission timeout when sending to a sleeping end device.
        SP also determines how long the parent will buffer a message for
        a sleeping child.
        Range 0x20 - 0xAF0 (Default: 20)
        """
        return self.app_config.get("coordinator", {}).get(
            "cyclic_sleep_period", "20")

    @property
    def coordinator_number_of_cyclic_sleep_periods(self):
        """
        Set/read the number of cyclic sleep periods used to calculate end
        device poll timeout. If an end device does not send a poll request
        to its parent coordinator or router within the poll timeout, the end
        device is removed from the child table.
        The poll timeout is calculated in milliseconds as
        (3 * SN * (SP * 10ms)), minimum of 5 seconds. i.e. if SN=15,
        SP=0x64, the timeout is 45 seconds.
        Range 0x1 - 0xFFFF (Default: 1)
        """
        return self.app_config.get("coordinator", {}).get(
            "number_of_cyclic_sleep_periods", "1")

    @property
    def coordinator_AD0_DIO0_configuration(self):
        """
        Configure options for the AD0/DIO0 line of the module. Options
        include: Enabling commissioning button functionality, Analog to
        Digital converter, Digital Input
        and Output.
        0 = Disabled
    ...
    def coordinator_DIO5_configuration(self):
        ...
        """
        return self.app_config.get("coordinator", {}).get(
            "DIO5_configuration", "1")

    @property
    def coordinator_DIO10_PWM0_configuration(self):
        """
        Configure options for the DIO10/PWM0 line of the module.
        Options include:
    ...
    def coordinator_DIO12_configuration(self):
        ...
        """
        return self.app_config.get("coordinator", {}).get(
            "DIO12_configuration", "0")

    @property
    def coordinator_pull_up_resistor_enable(self):
        """
        Set/read bitfield to configure internal pullup resistors status for
         I/O lines.
        ...
        """
        return self.app_config.get("coordinator", {}).get(
            "pull_up_resistor_enable", "1FFF")

    @property
    def coordinator_associate_LED_blink_time(self):
        """
        Set/read the Associate LED blink rate. This value determines the
        blink rate of the Associate/DIO5 pin if D5=1 and the module has
        started a network. Setting LT to 0 will use the default blink
        time (500ms).
        Range 0x0A - 0xFF (Default: 0)
        """
        return self.app_config.get("coordinator", {}).get(
            "associate_LED_blink_time", "0")

    @property
    def coordinator_rssi_pwm_timer(self):
        """
        Set/read PWM timer register. Set duration of PWM
        (pulse width modulation) signal output on the RSSI pin (P6). The
        signal duty cycle is updated with each received packet or APS
        acknowledgment and is shut off when the timer expires.
    ...
    def coordinator_device_options(self):
        """
        Bit0 - Reserved. Bit1 - Reserved. Bit2 - Reserved. Bit3 - Disable NULL
        Transport Key.
        Range 0x0 - 0xFF (Default: 1)
        """
        return self.app_config.get("coordinator",
                                   {}).get("device_options", "1")

    @property
    def coordinator_IO_sampling_rate(self):
        """
        Set the IO sampling rate to enable periodic sampling. If set >0, all
        enabled digital IO and analog inputs will be sampled and transmitted
        every IR milliseconds. IO Samples are transmitted to the address
        specified by DH+DL.
    ...
    def coordinator_digital_IO_change_detection(self):
        """
        Bitfield that configures which digital IO pins should be monitored for
        change detection. If a change is detected on an enabled digital IO pin,
        a digital IO sample is immediately transmitted to the address specified
        by DH+DL.
        Range 0x0 - 0xFFFF (Default: 0)
        """
        return self.app_config.get("coordinator", {}).get(
            "digital_IO_change_detection", "0")

    @property
    def coordinator_supply_voltage_high_threshold(self):
        """
        Set/read the supply voltage high threshold. If the supply voltage is
        above this threshold, the module will set the supply voltage high bit
        in the IO sample packet. The supply voltage is sampled every
         1.1 seconds.
        Range 0x0 - 0xFFFF (Default: 0)
        """
        return self.app_config.get("coordinator", {}).get(
            "supply_voltage_high_threshold", "0")
